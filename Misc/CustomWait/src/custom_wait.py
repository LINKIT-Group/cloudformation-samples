...
def eventrule_reinvoke(event, context):
    ...
    probe_results = {
        item: {'bucket_key': '/'.join([payload['S3Prefix'], item])}
    ...
    for item, conf in probe_results.items():
        try:
            # logger.info(f"trying:{payload['S3Bucket']}--{conf['bucket_key']}")
    ...
    bucket_region = \
        s3_client.get_bucket_location(Bucket=payload['S3Bucket'])['LocationConstraint']
